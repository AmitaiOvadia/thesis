...
class Augmentor:
    ...
    def augment(img, h_fl, v_fl, rotation_angle, shift_y_x, scale_factor):
        ...
        img = Augmentor.shift(img, shift_y_x)
    ...
    @staticmethod
    def shift(image, shift_y_x):
        shift_y, shift_x = shift_y_x
        # Roll along rows (vertical shift)
        rolled = np.roll(image, shift_y, axis=0)
        # Roll along columns (horizontal shift)
        rolled = np.roll(rolled, shift_x, axis=1)
        return rolled
